// GENERATED — do not edit. Source: lezer-pyret/bundle-entry.js (+ to-rnglr.js, namemap.json, pyret.named.grammar).
// Browser Lezer-for-Pyret frontend for code.pyret.org. Wrapped as an AMD module so the
// CPO build resolves it as the `lezer-pyret-frontend` native require (see cpo-config.json
// raw-js). Rebuild: cd lezer-pyret && ./node_modules/.bin/esbuild bundle-entry.js --bundle
//   --platform=browser --format=cjs --loader:.grammar=text --define:process.env.LEZER_TRACE=false
//   (with this banner/footer). Exports: { lezerParseToRnglr(pyretTokens, src) }.
define("lezer-pyret-frontend", [], function() {
  var module = { exports: {} };
...
function lezerParseToRnglr(pyretTokens, src) {
  if (false) process.stderr.write("[LEZER-FRONTEND] parsing " + src.length + " chars\n");
  build();
