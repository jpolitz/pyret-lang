...
- [!!] `type-structs.ts:469` `TRecord.key()` — insertion-order identity keys (cross-cutting #1; the sharpest single hazard in the port).
- [!] `type-structs.ts:552-634` `equals` — `E.EqualityResult` (with reason strings) collapsed to `boolean`; diagnostic reasons unrecoverable.
...
- [!!] Solve-loop iteration order (ts:509, 1050, 1080, 1121, 1232) — cross-cutting #1.
...
- [!!] `js-of-pyret.ts:21-30` `clMapSd` — emitted module-object field order (cross-cutting #1/#2).
