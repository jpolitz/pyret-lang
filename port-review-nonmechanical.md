...
- [!] ⏭️ `type-defaults.ts:814-830` — `defaultModules` is a shared **mutable** module-level Map returned by reference from `makeDefaultModules()` (Pyret froze it); any caller mutation corrupts all later `emptyContext()`s. Same pattern `makeDefaultTypes` ts:119-170. [⏭️ Verified inert: `TypingContext` is fully functional — every setter returns a fresh context and each `set*` caller uses `mapSet` (copy); grepped, there is no in-place `.modules.set(`/`.aliases.set(`/`.dataTypes.set(`. Nothing ever mutates the shared default maps, so the corruption can't occur (a TS-impl-choice: the functional-context invariant guards it).]
...
- [i] ts:222-224 `toString` renders "N binds" instead of full contents; ts:240 `torepr`→`toString`; ts:131 `RecordPath.toString` = JSON.stringify **and is used as a Structure dict key** (ts:158-160) [✔ not the TRecord.key() hazard — deterministic per fieldName, Path is semantically ordered, key is internal/self-consistent]; ts:707 `as ConstraintSystem` (TypeError vs field-not-found); `typeSetUnion` duplicated (ts:74-80 vs type-structs.ts:258).
...
- [!] ⏭️ ts:25-26 — `FrozenNameDict` = `NameDict` = mutable Map; freeze = copy, immutability convention-only. ts:1522-1526 — scrutinee widened by cast to keep Pyret's extra cases (faithful, type-unsound). [⏭️ See TS-impl-choice above: `FrozenNameDict` is a plain `Map` used read-only by convention (the `freevars*` results); nothing mutates a "frozen" dict, so the convention-only immutability holds. The ts:1522-1526 cast is a faithful (type-unsound) mirror of Pyret's extra cases.]
...
- [~] `js-ast.ts:22-149` — sourcemap layer reimplemented on npm `source-map`; `toUglySourcemap` returns `{code, map}`. Needs independent map-level verification. [✔ verified: emitted `theMap` byte-identical across compilers (compiled-module diff).]
- [!] ⏭️ `js-ast.ts:1103-1114` `JNum` — `String(this.n)` correct **iff** `n` is a js-numbers value; a raw JS float would emit `(1.5)` where Pyret emits `_makeNumberFromString("3/2")`. Invariant unenforced (only construction site gates on `typeof === 'number'` mirroring num-is-fixnum). [⏭️ Verified inert: compiled a program with `3/2`, `1.5`, `1/3`, `100` and the emitted JS is byte-identical across both compilers. Source literals always reach `JNum` as js-numbers values; a raw JS float would be a codegen bug, not user-reachable. TS-impl-choice.]
...
- [!] ✅ `compile-lib.ts:352-384` `structurallyEqual`/`unique` — hand-rolled deep-equality with a `key()` short-circuit (same constructor + equal `key()` ⇒ equal regardless of other fields) + first-occurrence ordering, replacing runtime structural sets; error dedup/order can differ. [`8ede7f0de`: This was worth fixing — the TS port was missing a `reverse` that was implicit in how Pyret traversed things. This commit makes sure the order of reported errors is the same across both. Easy to do, didn't actually trip any tests up, but useful to have the same. (Pyret `unique` = `list-to-list-set(...).to-list()`, which reverses first-occurrence order; multi-error programs were the exact reverse. Note it's the .arr's algorithmic order, NOT a source sort — a check-block error prints last regardless of line. Parity program `err-multi-unbound.arr` pins it.)]
...
- [!] ⏭️ mtime representation is threefold: `builtin.ts:66,112` + `jsfile.ts:24` fractional `mtimeMs`; `file.ts:95` integer `Number(stats.mtime)`; original floors to integer ms — matters at the `needsCompile` boundary comparisons (builtin.ts:150, 166). [⏭️ Cache-decision only, not output-observable. `needsCompile`/`getCompiled` (builtin.ts:150,166) compare `ctimes.mtimeMs`↔`stimes.mtimeMs` from the *same* `statSync` (fractional↔fractional, consistent). The threefold mix only meets in `depTimesFromWorklist`/`getCachedIfAvailable` → `cachedAvailable`, i.e. the CLI on-disk compiled-cache freshness check: it decides recompile-vs-reuse-cache, and the output is identical either way. vs Pyret's floored ms, TS could differ only at sub-millisecond boundaries → at worst a spurious recompile; the stale-cache race is pre-existing (the `getCompiled` NOTE) and unchanged. Not worth hardening.]
