({
  ...
  theModule: function(RUNTIME, ns, uri, fsInternal, loader, t, buffer) {
    ...
    function getBuiltinLocator(path) {
      ...
      // DIRECT-MODE OVERRIDE: fsInternal is the synchronous direct-mode
      // filesystem (sync thenables), so chain eagerly and unwrap via await.
      var chained = fsInternal.resolve(path + ".js").then(function(fullPath) {
        return fsInternal.readFile(fullPath);
      }).then(function(fileContents) {
        return builtinLocatorFromString(Buffer.from(fileContents).toString('utf8'));
      });
      return RUNTIME.await(chained);
    }
