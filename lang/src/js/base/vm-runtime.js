...
//      }
    ...
    ************************/
    ...
    function toReprLoop(val, reprMethods) {
          ...
          if (top.todo.length > 0) {
            ...
            if(isNumber(next)) { finishVal(reprMethods["number"](next)); }
              ...
              if(typeof objHasBeenSeen === "string") {
                ...
                // A suspended _output parks the whole worklist on its
                // promise; outputResume replays the cont toReprFun's
                // delivery step (unwrap, then valueskeleton-or-finish).
                if(isThenable(s)) { return s.then(outputResume); }
      ...
      function outputResume(v) {
        var fl = popFloors();
        if (fl !== null) { return fl.then(function() { return outputDeliver(v); }); }
        return outputDeliver(v);
      }
      function outputDeliver(v) {
        if (stack.length === 0) {
          thisRuntime.ffi.throwInternalError("Somehow we've drained the toRepr worklist, but have results coming back");
        }
        var top = stack[stack.length - 1];
        var a = thisRuntime.unwrap(v);
      ...
      function reenterToReprFun(val) {
        ...
        if (isThenable(ans)) {
          return ans.then(function(v) {
            var fl = popFloors();
            if (fl !== null) {
              return fl.then(function() { stack = stackOfStacks.pop(); return v; });
            }
...
// throw new Error("Internal error: tried to
            ...
            if (curPair !== false) {
              ...
              if (isRef(curLeft) && isRef(curRight)) {
                ...
                if (!sameBrands(getBrands(curLeft), getBrands(curRight))) {
                  ...
                  // A suspended _equals parks the worklist; the resume
                  // combines the delivered result and re-enters, as the
                  // cont equalFun's $ar path did.
                  if(isThenable(newAns)) { return newAns.then(equalsResume); }
      ...
      function equalsResume(v) {
        var fl = popFloors();
        if (fl !== null) { return fl.then(function() { return equalsDeliver(v); }); }
        return equalsDeliver(v);
      }
      function equalsDeliver(v) {
        toCompare.curAns = combineEquality(toCompare.curAns, v);
        return equalHelp();
      }
      function reenterEqualFun(left, right) {
        ...
        // The settle step is the cont reenterEqualFun's case 1: fix up the
        // optimistic Equal cache entries for the pairs this (re)entry
        // opened, then restore the outer comparison.
        function settle(ans) {
        ...
        if (isThenable(ans)) {
          return ans.then(function(v) {
            var fl = popFloors();
            if (fl !== null) { return fl.then(function() { return settle(v); }); }
            return settle(v);
          });
        }
        return settle(ans);
...
//      this.refinement = ann.refinement || this.refinement;
    ...
    */
    ...
    /*
      Promise-backend core (vm-runtime.js).

      Suspension is a thenable, not a Cont: a callee that pauses returns a
      real Promise, and every runtime helper that calls back into Pyret is
      thenable-aware (maybe-promise: synchronous unless something actually
      suspended). The fuel model is deliberately IDENTICAL to the cont
      runtime's, event for event -- every --GAS/--RUNGAS/++GAS the cont
      backend performs (generated entry checks, helper loops, the
      trampoline's capture refills and per-pop floors) has a counterpart
      here at the same point in execution, and refills consult the same
      pause-schedule getters. That is what makes "same schedule => same
      pauses with the same stacks" hold against the cont backend by
      construction (see VM_DESIGN.md at the repo root).
    */

    // Duck-typed rather than instanceof Promise: cross-realm promises (the
    // browser loader) must be caught; a false negative leaks a raw Promise
    // into the next Pyret operation as "Non Pyret value".
    function isThenable(res) {
      return res !== null && typeof res === "object" && typeof res.then === "function";
    }

    // Mirrors the cont run()'s per-run closure state, hoisted so helpers
    // and the machine can reach it between calls.
    var CUR_SYNC = false;
    var defaultInitialGas = 500;
    var defaultInitialRunGas = 5000;
    ...
    function macroYieldP() {
      return new Promise(function(resolve) { util.suspend(resolve); });
    }

    // Bumped at every capture event (a fuel-check trip, or pauseStack).
    // A helper resuming from a .then compares its snapshot against this:
    // stale means "a capture happened while I was parked", i.e. the cont
    // backend would have popped my ActivationRecord through iter, so the
    // per-pop floors apply before I continue.
    var CAPTURE_EPOCH = 0;

    // The chain of live machine states, outermost first, maintained by
    ...
    */
    ...
    /*
      ...
      Replicates iter's bounce sequence: EXN_STACKHEIGHT reset happened at
      the trip site; refill BOTH counters via the schedule getters; then a
      macrotask yield (util.suspend) unless the run is sync. The returned
      promise resolves when execution should continue; the *initiating*
      frame's pop-floors + re-entry decrement are the caller's job (they
    ...
    */
        ...
        if (manualPause !== null) {
          ...
          return pauseStack(function(restarter) {
            thePause.setHandlers({
              resume: function() { restarter.resume(undefined); },
    ...
    /*
      iter's per-pop floor sequence (runtime.js keeps GAS and RUNGAS >= 2
      at every frame pop so a resumed entry check cannot re-trip and
      clobber the delivered answer). Returns null when everything was
      synchronous, else a promise. Callers run it only when a capture
      intervened (epoch went stale) -- a frame that was never captured is
      never popped through iter on the cont side either.
    */
    function popFloors() {
      if (!CUR_SYNC && thisRuntime.RUNGAS <= 1) {
        thisRuntime.RUNGAS = nextInitialRunGas();
        return macroYieldP().then(function() {
          if (thisRuntime.GAS <= 1) { thisRuntime.GAS = nextInitialGas(); }
          return null;
        });
      }
      if (thisRuntime.GAS <= 1) { thisRuntime.GAS = nextInitialGas(); }
      if (CUR_SYNC && thisRuntime.RUNGAS <= 1) { thisRuntime.RUNGAS = nextInitialRunGas(); }
      return null;
    }

    /*
      safeCall: run `fun`, then `after(result)`. Maybe-promise -- fully
      synchronous when nothing suspends, which _checkAnn's cheap path
      depends on. Fuel mirrors the cont safeCall exactly: the entry check
      (which also re-runs on every resume, as the cont AR path does), and
      ++GAS on completion.
    */
    function safeCall(fun, after, stackFrame) {
      if (--thisRuntime.GAS <= 0 || --thisRuntime.RUNGAS <= 0) {
        thisRuntime.EXN_STACKHEIGHT = 0;
        return captureEventP().then(function() {
          var fl = popFloors();
          if (fl !== null) { return fl.then(function() { return safeCall(fun, after, stackFrame); }); }
          return safeCall(fun, after, stackFrame);
        });
      }
      var funRes = fun();
    ...
    /*
      eachLoop: cont charged --GAS at (re)entry to restart(), --RUNGAS per
      iteration, ++GAS at the clean exit; a resumed AR re-ran restart()
      from the top. Same charges here.
    */
    function eachLoop(fun, start, stop) {
      ...
      function restart() {
        if (--thisRuntime.GAS <= 0) {
          thisRuntime.EXN_STACKHEIGHT = 0;
          return captureEventP().then(resumeRestart);
        }
        for (;;) {
    ...
    // Set by breakAll; the next capture event throws userBreak into the
    // running chain (the promise counterpart of the trampoline's dead
    // thread going quiet).
    var BREAK_FLAG = false;

    /*
      The promise-backend driver. Same external contract as the cont run():
      RUN_ACTIVE guard, thread bookkeeping (execThunk starts nested runs
      through pauseStack), onDone with Success/FailureResult carrying
      getStats(). The trampoline is gone: the program is a maybe-promise
      and we settle it.
    */
    function run(program, namespace, options, onDone) {
      ...
      defaultInitialRunGas = options.initialRunGas || defaultInitialGas * 10;
      // The getters are consulted here in the same order the cont run()
      // consults them, so a stateful schedule sees the same call sequence.
      ...
      try {
        var val = program(thisRuntime, namespace);
        if (isThenable(val)) {
          val.then(finishSuccess, handleErr);
        } else {
          finishSuccess(val);
        }
      } catch(e) {
    ...
    /*
      Promise-backend pauseStack: instead of a Pause continuation riding
      the capture path, return a real Promise and hand the resumer a
      restarter whose resume/error/break settle it. A pause is a capture
      event for the machine (every live frame is marked, at-return frames
      elided) exactly as the cont unwind attaches every live frame.
      RUN_ACTIVE is released while paused so a nested run (execThunk) can
      start, and restored on any settlement; resume refills both counters
      via the schedule getters and re-enters through a macrotask, as the
    ...
    */
    ...
    /*
      raw-array-build kept the cont version's fuel exactly: --GAS on every
      (re)entry, --RUNGAS per iteration (with the length test first, as the
      while condition ordered them), no ++GAS refund. `step` 1 means a
      resumed callback value is pending in ansV, delivered before the next
      call -- the AR's $step/$ans, made explicit.
    */
    var raw_array_build = function(f, len) {
      ...
      var curIdx = 0;
      var arr = new Array();
      function resumeEnter(step, ansV) {
        var fl = popFloors();
        if (fl !== null) { return fl.then(function() { return enter(step, ansV); }); }
        return enter(step, ansV);
      }
      function enter(step, ansV) {
        if (--thisRuntime.GAS <= 0) {
          thisRuntime.EXN_STACKHEIGHT = 0;
        ...
        while (curIdx < len) {
          ...
          if (isThenable(res)) {
            return res.then(function(v) { return resumeEnter(1, v); });
          }
          arr.push(res);
          curIdx++;
        }
    ...
    var raw_array_fold = function(f, init, arr, start) {
      ...
      // Fuel is the cont version's: --RUNGAS per iteration, nothing else;
      // a resume applies the trampoline's pop floors, delivers the value
      // into the accumulator (the foldFun $ar.ans path), and re-enters.
      function foldHelp() {
        while(currentIndex < (length - 1)) {
          if(--thisRuntime.RUNGAS <= 0) {
            thisRuntime.EXN_STACKHEIGHT = 0;
            return captureEventP().then(foldResumePause);
          }
          currentIndex += 1;
          var res = f.app(currentAcc, arr[currentIndex], currentIndex + start);
          if(isThenable(res)) { return res.then(foldResumeValue); }
          currentAcc = res;
        }
        return currentAcc;
      }
      function foldResumePause() {
        var fl = popFloors();
        if (fl !== null) { return fl.then(foldHelp); }
        return foldHelp();
      }
      function foldResumeValue(v) {
        var fl = popFloors();
        if (fl !== null) { return fl.then(function() { currentAcc = v; return foldHelp(); }); }
        currentAcc = v;
        return foldHelp();
      }
      return foldHelp();
    ...
    var raw_array_bool_mapper = function(name, good, bad) {
      return function(f, arr, start) {
        ...
        function foldResumeValue(v) {
          var fl = popFloors();
          if (fl !== null) { return fl.then(function() { return foldResumeCheck(v); }); }
          return foldResumeCheck(v);
        }
        function foldResumeCheck(v) {
          if (v === bad) { return v; }
          return foldHelp();
        }
        return foldHelp();
    ...
    var raw_array_map = function(f, arr) {
      ...
      function mapHelp() {
        while(currentIndex < (length - 1)) {
          if(--thisRuntime.RUNGAS <= 0) {
            thisRuntime.EXN_STACKHEIGHT = 0;
            return captureEventP().then(mapResumePause);
          }
          currentIndex += 1;
          var res = f.app(arr[currentIndex]);
          if(isThenable(res)) { return res.then(mapResumeValue); }
          newArray[currentIndex] = res;
        }
        return newArray;
      }
      function mapResumePause() {
        var fl = popFloors();
        if (fl !== null) { return fl.then(mapHelp); }
        return mapHelp();
      }
      function mapResumeValue(v) {
        var fl = popFloors();
        if (fl !== null) { return fl.then(function() { newArray[currentIndex] = v; return mapHelp(); }); }
        newArray[currentIndex] = v;
        return mapHelp();
      }
      return mapHelp();
    };
    ...
    var raw_array_each = function(f, arr) {
      ...
      function eachHelp() {
        while(currentIndex < (length - 1)) {
          if(--thisRuntime.RUNGAS <= 0) {
            thisRuntime.EXN_STACKHEIGHT = 0;
            return captureEventP().then(eachResume);
          }
          currentIndex += 1;
          var res = f.app(arr[currentIndex]);
          if(isThenable(res)) { return res.then(eachResume); }
        }
        return nothing;
      }
      function eachResume() {
        var fl = popFloors();
        if (fl !== null) { return fl.then(eachHelp); }
        return eachHelp();
      }
      return eachHelp();
    };
    ...
    var raw_list_map = function(f, lst) {
      ...
      function foldResumeValue(v) {
        var fl = popFloors();
        if (fl !== null) { return fl.then(function() { currentAcc.push(v); return foldHelp(); }); }
        currentAcc.push(v);
        return foldHelp();
      }
      return foldHelp();
    ...
    var raw_list_filter = function(f, lst) {
      ...
      // The resumed value goes through the same raw truthiness test the
      // cont version's $ar.ans path used (no Boolean check on this path).
      function foldResumeValue(v) {
        var fl = popFloors();
        if (fl !== null) { return fl.then(function() { return foldResumeApply(v); }); }
        return foldResumeApply(v);
      }
      function foldResumeApply(v) {
        if(v) {
          currentAcc.push(currentFst);
        }
        return foldHelp();
    ...
    var raw_array_filter = function(f, arr) {
      ...
      function filterHelp() {
        while(currentIndex < (length - 1)) {
          if(--thisRuntime.RUNGAS <= 0) {
            thisRuntime.EXN_STACKHEIGHT = 0;
            return captureEventP().then(filterResumePause);
          }
          currentIndex += 1;
          var res = f.app(arr[currentIndex]);
          if(isThenable(res)) { return res.then(filterResumeValue); }
          if(!(isBoolean(res))) {
      ...
      function filterResumePause() {
        var fl = popFloors();
        if (fl !== null) { return fl.then(filterHelp); }
        return filterHelp();
      }
      // Same raw truthiness on the resumed value as the cont $ar.ans path.
      function filterResumeValue(v) {
        var fl = popFloors();
        if (fl !== null) { return fl.then(function() { return filterResumeApply(v); }); }
        return filterResumeApply(v);
      }
      function filterResumeApply(v) {
        if(v) { newArray.push(arr[currentIndex]); }
        return filterHelp();
      }
      return filterHelp();
    };
    ...
    var raw_list_fold = function(f, init, lst) {
      ...
      function foldHelp() {
        while(thisRuntime.ffi.isLink(currentLst)) {
          ...
          if(isThenable(currentAcc)) { return currentAcc.then(foldResumeValue); }
        }
        return currentAcc;
      }
      function foldResumePause() {
    ...
    var thisRuntime = {
      ...
      '$popFloors': popFloors,
      '$vmStateChain': VM_STATE_CHAIN,
      '$nextInitialGas': nextInitialGas,
