...
function requiresOf(imports: A.Import[]): J.JExprT {
  ...
  const entries = sorted.map((i) => {
    ...
    switch (m.$name) {
      ...
      default:
        throw new InternalCompilerError('interp-of-pyret: unknown Dependency');
    }
  });
...
export function makeInterpPyret(
...
export function traceMakeInterpPyret(
  addPhase: (name: string, value: any) => any,
  programAst: A.Program,
  env: C.CompileEnvironment,
  postEnv: C.ComputedEnvironment,
  ...
  const [flatProvides, ccp] = makeInterpPyret(
