...
// server.js / src/ts-compiler/src/server.ts).
//
// Usage: node serve-smoke.js <socket-path> <program.arr> <outfile.jarr> [--bad]
//
...
// 1 on compile-failure, 3 on close-without-result.
...
const pyretOptions = {
  'program': program,
  'outfile': outfile,
  'base-dir': path.dirname(path.resolve(program)),
