...
# Driver: per frame, update (ticker) then draw (drawer); image-width forces the
# overlay tree to be realized. acc accumulates it so nothing is elided.
fun run-frames(n, w, acc):
  if n == 0: acc
  else:
    w2 = ticker(w)
    img = drawer(w2)
    run-frames(n - 1, w2, acc + I.image-width(img))
  end
end

final-acc = run-frames(3000, world0, 0)
print(num-floor(final-acc))
print("\n")
