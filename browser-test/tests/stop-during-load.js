 ...
 * it).
 *
 * Stop is pressed 3s in. It cannot be pressed sooner: repl-ui.js's
 * setWhileRunning enables #breakButton on a RUNNING_SPINWHEEL_DELAY_MS (1000ms)
 * timer, so for the first second of any run there is nothing to click.
 *
...
// Long enough that the fetch is unambiguously still open when we click, short
// enough that the test is not slow. Stop lands halfway through.
const HOLD_MS = 8000;
...
module.exports = function registerStopDuringLoad(getSession) {
  test("Stop pressed while an import is still loading leaves the editor usable",
    { timeout: 120000 }, async () => {
      ...
      await new Promise((r) => setTimeout(r, CLICK_AT_MS));
      const stop = await page.eval(CLICK_STOP);
      if (!stop.present || !stop.wasEnabled) {
        throw new ProceduralError(
          "Stop was not clickable " + CLICK_AT_MS + "ms into the run, so this test never " +
      ...
      await new Promise((r) => setTimeout(r, (HOLD_MS - CLICK_AT_MS) + 6000));

      const shown = await page.eval(OUTPUT_TEXT);
      assert.ok(
        shown.indexOf(IMPORTED_VALUE) === -1,
        "Stop did not stop the program. " + CLICK_AT_MS + "ms into a run whose import was " +
