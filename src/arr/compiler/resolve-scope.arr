      ...
      imports-and-env = for fold(
        ...
        cases(A.Import) i:
          | s-import-complete(l2, vnames, tnames, file, name-vals, name-types) =>
            atom-env = make-atom-for(name-vals, false, acc.e, bindings, let-bind(_, _, A.a-blank, some(i)))
            atom-env-t = make-atom-for(name-types, false, acc.te, type-bindings, let-type-bind(_, _, some(i)))
